function getIdmClientDetails() {
    return {
        ...
        "endpoint": systemEnv.getProperty("esv.identity.cloud.baseurl") + "/am/oauth2/realms/root/realms/" + systemEnv.getProperty("esv.sapig.identity.cloud.realm")  + "/access_token",
        "scope": "fr:idm:*",
...
function fetchIntentFromIdm(intentId, intentType) {
    ...
    var request = new org.forgerock.http.protocol.Request();
    var uri = encodeURI(systemEnv.getProperty("esv.identity.cloud.baseurl") + "/openidm/managed/" + intentType + "/" + intentId + "?_fields=_id,_rev,OBIntentObject,user/_id,accounts,apiClient/_id")
